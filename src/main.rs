...
impl Order {
    fn new() -> Order {
        Order { dishes: HashMap::new(), is_takeaway: false }
    }

    fn add_dish(&mut self, dish: Dish) {
        let d = self.dishes.get(&dish).cloned();
    ...
    fn total(&self) -> u32 {
        let sum = self.dishes.iter().fold(0, |s, (x, y)| s + x.price() * y);
...
impl VanBinh {
    pub fn new() -> VanBinh {
        VanBinh { orders_count: 1, customers: vec![] }
    }
